...
//////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  public class WebSocketServer: Server, ConnectionDelegate {
    ...
    private let queue = DispatchQueue(
      label: "com.vluxe.starscream.server.networkstream", attributes: [])
    ...
    public func start(address: String, port: UInt16) -> Error? {
      ...
      parameters.requiredLocalEndpoint = NWEndpoint.hostPort(
        host: NWEndpoint.Host.name(address, nil), port: p)

      guard let listener = try? NWListener(using: parameters, on: p) else {
        return WSError(
          type: .serverError, message: "unable to start the listener at: \(address):\(port)",
          code: 0)
      }
  ...
  @available(macOS 10.14, iOS 12.0, watchOS 5.0, tvOS 12.0, *)
  public class ServerConnection: Connection, HTTPServerDelegate, FramerEventClient,
    FrameCollectorDelegate, TransportEventClient
  {
    ...
    public func connectionChanged(state: ConnectionState) {
      ...
      case .failed(let error):
        NSLog(
          "[NT4] Server connection error: \(error ?? WSError(type: .protocolError, message: "Default error, no extra data", code: 0))"
        )  //handleError(error)
      case .viability(_):
        break
      ...
      case .cancelled:
        NSLog("[NT4] Server connection cancelled!")
      //broadcast(event: .cancelled)
      case .peerClosed:
        delegate?.didReceive(
          event: .disconnected(
            self, "Connection closed by peer", UInt16(FrameOpCode.connectionClose.rawValue)))
      }
    }
