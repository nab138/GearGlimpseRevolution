...
//
// https://github.com/maxxfrazer/SceneKit-SCNLine/blob/main/Sources/SCNLine/SCNGeometry%2BExtensions.swift
...
extension simd_quatf {
  fileprivate func act(_ vector: SCNVector3) -> SCNVector3 {
    let vec = self.act(SIMD3<Float>([vector.fx, vector.fy, vector.fz]))
    return SCNVector3(Float(vec.x), Float(vec.y), Float(vec.z))  // Use Floats
  }
  fileprivate func split(by factor: Float = 2) -> simd_quatf {
    if self.angle == 0 {
      return self
    } else {
  ...
  fileprivate static func zero() -> simd_quatf {
...
extension SCNGeometry {
  ...
  public static func buildGeometry(
  ...
  public static func getAllLineParts(
    points: [SCNVector3], radius: Float, edges: Int = 12,
    maxTurning: Int = 4
    ...
    for (index, point) in points.enumerated() {
      ...
      if index == 0 {
        ...
        cPoints = SCNGeometry.getCircularPoints(
          radius: radius, edges: edges, orientation: initialRotation)
        ...
        if point.distance(vector: points[index - 1]) > radius * 2 {
          ...
          if mTurn > 1 {
            ...
            for i in 0..<Int(mTurn) {
              ...
              SCNGeometry.addCylinderVerts(
                to: &trueInds, startingAt: trueVs.count - edges * 4, edges: edges)
    ...
    return (
      GeometryParts(vertices: trueVs, normals: trueNormals, uvs: trueUVMap, indices: trueInds),
      lineLength
    )
  }
