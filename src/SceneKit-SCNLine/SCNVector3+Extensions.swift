...
extension SCNVector3 {
  ...
  func rotate(about origin: SCNVector3, by: Float) -> SCNVector3 {
    ...
    let cosAngle = cos(by)
    return SCNVector3(
      x: Float(pointRepositionedXY[0] * cosAngle - pointRepositionedXY[1] * sinAngle + origin.fx),
      y: self.y,
      z: Float(pointRepositionedXY[0] * sinAngle + pointRepositionedXY[1] * cosAngle + origin.fz)
    )
  }
}
...
internal func * (vector: SCNVector3, scalar: Float) -> SCNVector3 {
  return SCNVector3Make(
    Float(vector.fx * scalar), Float(vector.fy * scalar), Float(vector.fz * scalar))
}
